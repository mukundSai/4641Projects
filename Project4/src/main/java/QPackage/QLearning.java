 ...
 * the latter of which is performed by running many learning episodes in succession in a {@link SimulatedEnvironment}.
 ...
 * anything with the {@link #setLearningPolicy(Policy)} policy.
 * <p>
 * If you
 * want to use a custom learning rate decay schedule rather than a constant learning rate, use the
 * {@link #setLearningRateFunction(LearningRate)}.
 * <p>
...
public class QLearning extends MDPSolver implements QProvider, LearningAgent, Planner{
	 ...
	 * @param gamma the discount factor
	 * @param hashingFactory the state hashing factory to use for Q-lookups
	 * @param qInitFunction a {@link QFunction} object that can be used to initialize the Q-values.
	 * @param learningRate the learning rate
	 * @param learningPolicy the learning policy to follow during a learning episode.
	 * @param maxEpisodeSize the maximum number of steps the agent will take in a learning episode for the agent stops trying.
	 */
	...
	protected QValue getQ(HashableState s, Action a) {
		if(this.model.terminal(s.s())){
			return new QValue(s.s(),a,rf.reward(s.s(),new SimpleAction(),s.s()));
		}
		QLearningStateNode node = this.getStateNode(s);
	 ...
	 * is stored, then it is created and has its Q-value initialize using this objects {@link QFunction} data member.
	...
	protected double getMaxQ(HashableState s){
		...
		if(this.model.terminal(s.s())){
			return rf.reward(s.s(),new SimpleAction(),s.s());
		}
