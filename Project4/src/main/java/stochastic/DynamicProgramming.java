...
public class DynamicProgramming extends MDPSolver implements ValueFunction, QProvider {
	...
	/**
	 * Common init method for {@link DynamicProgramming} instances. This will automatically call the
	 * {@link MDPSolver#solverInit(SADomain, double, HashableStateFactory)}
	 * method.
	...
	public double value(HashableState sh){
		if(this.model.terminal(sh.s())){
			return rf.reward(sh.s(),new SimpleAction(),sh.s());
		}
	...
	protected double performBellmanUpdateOn(HashableState sh){
		if(this.model.terminal(sh.s())){
			valueFunction.put(sh, rf.reward(sh.s(),new SimpleAction(),sh.s()));
			return rf.reward(sh.s(),new SimpleAction(),sh.s());
		}
	 ...
	 * *is* compatible with {@link Option} objects.
