package stochastic.policyiteration;
...
import stochastic.DynamicProgramming;
...
public class PolicyIteration extends DynamicProgramming implements Planner {
	...
	public PolicyIteration(SADomain domain, double gamma, HashableStateFactory hashingFactory, double maxDelta, int maxEvaluationIterations, int maxPolicyIterations, boolean printData){
		this.DPPInit(domain, gamma, hashingFactory);
	 ...
	 * the evaluative policy will be {@link GreedyQPolicy} on the function evaluation.
	...
	/**
	 * Plans from the input state and then returns a {@link GreedyQPolicy} that greedily
	 * selects the action with the highest Q-value and breaks ties uniformly randomly.
	 * @param initialState the initial state of the planning problem
	 * @return a {@link GreedyQPolicy}.
	 */
	@Override
	...
	protected double evaluatePolicy(){
		...
		int i;
		System.out.println("Iteration Num, Average Reward");
	...
	public void printIterationData(int i) {
		double numTrials = 1;
		double totalReward = 0;
		for (int j = 0; j < numTrials; j++) {
			Episode episode = PolicyUtils.rollout(new GreedyQPolicy(this), new EXGridState(2, 1), domain.getModel());
			double sum = 0;
		...
		System.out.println(i + "," + totalReward/numTrials);
	}
